...
"dpkg":http://www.debian.org/doc/FAQ/ch-pkgtools.en.html or make use of the "apt repository that the RabbitMQ team provides":http://www.rabbitmq.com/debian.html#apt.
...
a message published with a routing key of "americas.north.us.ca.berkeley" would be routed to several queues: "us.california" and the
...
The name of server-named queue is generated by the broker and sent back to the client with a queue declaration confirmation.
...
callbacks. We *strongly recommend that you always use a callback for server-named queues*, otherwise your code may be subject
to "race conditions":http://en.wikipedia.org/wiki/Race_condition. Even though the amqp gem tries to be reasonably smart and protect you from most common problems, there
is no way it can do so for every case. The only reason we support {AMQP::Channel#queue} usage without a callback for server-named queues is for
...
They most important line in this example is
...
Here is one example using "RSpec":http://relishapp.com/rspec-style
...
Please take a moment to tell us what you think about this guide "on Twitter":http://twitter.com/rubyamqp or the "Ruby AMQP mailing list":http://groups.google.com/group/ruby-amqp.
 Let us know what was unclear or what has not been covered. Maybe you do not like the guide style or grammar and discover spelling mistakes. Reader feedback is
