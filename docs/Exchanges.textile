...
Since direct exchanges use *message routing key* for routing, message producers need to specify it:
<pre>
<code>
...
The routing key then will be compared [for equality] with routing keys on bindings and consumers that subscribed with
...
a round robin manner. When doing so, it is important to understand that in AMQP 0.9.1, *messages are load balanced between
consumers, not between queues*.

Ruby amqp gem historically has a limitation that only one consumer (message handler) is allowed per one {AMQP::Queue} instance.
This limitation will be addressed in the future, but with amqp gem 0.8.x, if you want to load balance messages between multiple
consumers in the same application/OS process, you need to use a separate channel for each of them.

{file:docs/Queues.textile Working With Queues} and {file:docs/PatternsAndUseCases.textile Patterns and Use Cases} guides provide more information on this subject.
...
Routing example above can be rewritten to use the default exchange:
 ...
 * Direct (near real-time) messages to individual players in a MMO game.
 ...
 * Passing data between workflow steps, each having an identifier (also consider using headers exchange for that).
...
<dl>
  ...
  <dt>:reply_to</dt>
  <dd>
    Commonly used to name a reply queue (or any other identifier that helps consumer application to direct its response).
    Applications are encouraged to use this attribute instead of putting this information into message payload.
  </dd>
  ...
  <dt>:headers</dt>
  <dd>Ruby hash of any additional attributes application needs. Nested hashes are supported.</dd>
</dl>

Application authors are recommended to use well-known message attributes when applicable instead of relying on custom headers or placing
information into message body. For example, if your application messages have priority, publishing timestamp, type and content type,
you should use respective AMQP message attributes instead of reinventing the wheel.
...
"validated User ID":http://www.rabbitmq.com/extensions.html#validated-user-id. If this property is set by a publisher, its value must be equal to the name of the user used to open the connection.
...
h3. Publishing messages as persistent.

Messages potentially spend some time in queues where they were routed before they are consumed. During this period of time, broker may crash or
experience a restart. To survive it, messages must be persisted to disk. This has negative effect on performance, especially with network attached
storage like NAS devices and Amazon EBS. AMQP 0.9.1 lets applications trade off performance for durability or vice versa, on a message-by-message basis.

To publish message as persistent, use :persistent option {AMQP::Exchange#publish} accepts:
...
<span class="note">
Note that in order to survive a broker crash, both message and queue it was routed to must be persistent/durable.
</span>
...
upon binding. Whether it is "any header" or "all of them" depends on 'x-match' header value.
In case of multiple queues, headers exchange will deliver a copy to each queue, just like direct exchanges do. Distribution rules between consumers
on a particular queue are the same as for direct exchange.
...
Headers exchanges can be looked at as "direct exchanges on steroids". Because they route based on header values, they can be used as
direct exchanges where routing key doesn't have to be a string: it can be an integer or a hash (dictionary), for example.
