...
like Chef or Puppet, misconfigurations can happen. Also, the broker might be down for some reason. Ideally, error detection should happen as early as possible. There are two ways of detecting
...
Phusion Passenger is also a pre-forking server, and just as with Unicorn, the EventMachine reactor and AMQP connection should be started *after* it forks worker
processes. Passenger documentation has "a section":http://bit.ly/passenger-forking-gotchas that explains how to avoid problems related to the behavior of the fork(2) system
call, namely:
...
Basically, the recommended default smart spawn mode works exactly the same as in Unicorn (with all the same common pitfalls). An "example application":http://bit.ly/ruby-amqp-gem-example-with-ruby-on-rails-and-passenger is available on github.
...
There is no need to start the EventMachine reactor. However, depending on application server, its version, the version of the framework and Rack middleware being used,
...
So, in case the EventMachine reactor is not running yet on server/application boot, the connection will not fail but will instead wait for the reactor to start.
