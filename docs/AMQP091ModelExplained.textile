...
h2. High-level overview of AMQP 0.9.1 & the AMQP Model
...
Messaging solutions have been around since the 1970s, with multiple products from multiple vendors being completely
or largely incompatible with each other. Because of this integration costs were very high (in double digits per cent) for many
large companies, for example, in the financial services industry.

AMQP 0.9.1 design pursued multiple goals but two key of them are

 * Produce an open standard for messaging middleware
 * Enable interoperability between various technologies and platforms

There is a lot of software running on many operating systems built with multiple programming languages running on
various hardware architectures and virtual machines. AMQP implementations like RabbitMQ make it possible for C
systems interoperate with Java, .NET, PHP or Ruby applications as well as Clojure, Erlang, JavaScript and Python.


h3. Brokers and their role

Messaging mrokers receive messages from _producers_ (applications that publish them) and route them to _consumers_ (applications
that process them).
