...
Second approach is more "correct", but at the same time a lot more tedious, because there are a lot of things to wrap your head around: initial setup, error handling,
...
(based on arvicco's amqp-spec and tmmm1's em-spec) provides various helpers for your asynchronous applications.
...
When you include one of the evented-spec helper modules, some magic happens: everything is wrapped in a <code>EM.start</code> + <code>AMQP.connect</code>
methods, so you can start writing your examples as if you're connected. You still need to initialize your own channel though.
...
What's important for us is that you *cannot use <code>sleep</code> for delays*. Why? Because all the specs code is processed directly in the reactor thread, if you
<code>sleep</code> in the reactor thread, it cannot send frames. What you need to use instead is <code>#delayed</code> method. It takes delay time in seconds and callback
...
In the following example, we declare two channels, then declare queue with the same name but different options (which raises a channel-level exception in AMQP):
