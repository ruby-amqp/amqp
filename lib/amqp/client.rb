...
module AMQP
  ...
  module Client
    ...
    def self.parse_amqp_url(amqp_url)
      ...
      raise("amqp:// uri required!") if !['amqp', 'amqps'].include? uri.scheme
      opts = {}
      opts[:user] = URI.unescape(uri.user) if uri.user
      opts[:pass] = URI.unescape(uri.password) if uri.password
      opts[:vhost] = URI.unescape(uri.path) if uri.path
      opts[:host] = uri.host if uri.host
      opts[:port] = uri.port ? uri.port : (uri.scheme == "amqps" ? 5671 : 5672)
      opts[:ssl] = true if uri.scheme == "amqps"
      opts[:logging] = true if q['logging'][0] == "true"
