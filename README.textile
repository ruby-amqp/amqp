...
Note that for Ubuntu and Debian it is a good idea to use "RabbitMQ apt repository":http://www.rabbitmq.com/debian.html#apt
that has recent versions of RabbitMQ. Ubuntu (even Karmic) and Debian both "ship with RabbitMQ 1.6.0":http://packages.ubuntu.com/karmic/rabbitmq-server,
which was released in July 2009 and only supports AMQP 0.8.0. amqp gem 0.8.0 and later *will not work with RabbitMQ versions
before 2.0*.
